...
import { Clock, Users, Fish, Waves, Sun, Anchor, ChevronDown, Phone, MessageCircle } from 'lucide-react';
...
export default function ExperienceCardsFixed({ onSelectExperience, selectedBoat, location }) {
      ...
      (boat.available_expeditions || []).forEach(expId => {
        if (!expMap.has(expId)) {
          ...
          if (catalog && !isHiddenForOperator(catalog, boat.operator)) {
            ...
            const displayDuration = pricing?.duration_hours ? `${pricing.duration_hours} hours` : (catalog.duration || `${duration_hours} hours`);
            expMap.set(expId, { ...catalog, title: catalog.title || expId.replace(/_/g, ' '), duration_hours, duration: displayDuration });
          }
        }
      });
    });
  ...
  if (selectedBoat) {
    const boatExperiences = (selectedBoat.available_expeditions || []).map(expType => {
      ...
      const displayDuration = pricing?.duration_hours ? `${pricing.duration_hours} hours` : (catalog?.duration || `${durationHours} hours`);
      ...
      <section className="relative py-8 md:py-12">
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
          <div className="rounded-3xl overflow-hidden border border-white/20 bg-white/5 backdrop-blur-md p-8 md:p-10">
            ...
            <div className="mt-12 bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8 backdrop-blur-xl">
  ...
  return (
    <section className="relative py-8 md:py-12 overflow-hidden" style={{ backgroundImage: `url('https://media.base44.com/images/public/6987f0afff96227dd3af0e68/c6ed2e8cf_FILUMarine2.png')`, backgroundRepeat: 'repeat', backgroundSize: '300px 300px', backgroundColor: '#081830' }}>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Direct Booking Cards */}
        <div className="mt-8 bg-white/5 border border-white/10 rounded-3xl p-6 md:p-8 backdrop-blur-xl">
          <div className="text-center mb-6">
            <h3 className="text-2xl font-light text-white mb-2">Prefer to book directly?</h3>
            <p className="text-white/60">Our team is ready to help you plan your perfect experience.</p>
          </div>
