...
import { Plus, Edit, Trash2, Ship, Check, X, Gauge, Package, ChevronDown, ChevronUp, Calendar, Wrench, MapPin, Users } from 'lucide-react';
...
export default function BoatManagement() {
  ...
  const [formData, setFormData] = useState({
    ...
    expedition_pricing: [],
    equipment: { // Updated structure for equipment
      bathroom: false,
      live_well: false,
      starlink: false,
      cctv: false,
    ...
    custom_equipment: [], // New field for custom equipment
    engine_config: '',
    engine_name: '',
    engine_quantity: 1,
    engine_year: null,
    ...
    mechanic_phone: '',
    mechanic_email: '',
    supply_seller_name: '', // This might be old, should be supply_sellers array
    supply_seller_phone: '', // This might be old
    supply_seller_email: '', // This might be old
    owner_phone: '',
    maintenance_schedule: '', // This might be old
    last_service_date: '',
  ...
  const [newEquipment, setNewEquipment] = useState(''); // State for new custom equipment input
  ...
  const resetForm = () => {
    setFormData({
      ...
      available_expeditions: [],
      expedition_pricing: [],
      equipment: { // Reset to default equipment state
        bathroom: false,
        live_well: false,
        starlink: false,
        cctv: false,
      ...
      custom_equipment: [], // Reset custom equipment
      engine_config: '',
      engine_name: '',
      engine_quantity: 1,
      engine_year: null,
    ...
    setNewEquipment(''); // Reset new equipment input
    setEditingBoat(null);
    setDialogOpen(false);
  };
  ...
  const handleEdit = (boat) => {
    ...
    setFormData({
      ...
      // Handle equipment: if it's an old array, it needs to be transformed, otherwise use as is or default
      equipment: boat.equipment && !Array.isArray(boat.equipment) ? boat.equipment : {
        bathroom: false,
        live_well: false,
        starlink: false,
        cctv: false,
        ...
        fishing_gear: false,
        snorkeling_gear: false,
        // If old equipment was an array, you might want to convert some entries here
        // For now, defaulting to false
      },
      custom_equipment: boat.custom_equipment || [], // Initialize custom equipment
    });
  ...
  // New function to toggle predefined equipment
  ...
  // New functions for custom equipment
                           ...
                           <div className="space-y-4 mt-3 pl-8">
                             ...
                             <div>
                               ...
                               <PickupAndDeparture 
                                 ...
                                 expeditionType={exp}
                                 location={formData.location} // New prop
                               />
                             </div>
                           </div>
                         )}
              ...
              {isExpanded && isRentalMode && (
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {boat.equipment && Object.entries(boat.equipment).filter(([, v]) => v).map(([key]) => (
                    ...
                    {boat.custom_equipment && boat.custom_equipment.map((eq, idx) => (
                      <span key={idx} className="text-xs px-2 py-1 bg-cyan-50 text-cyan-700 rounded capitalize">
                        {eq}
                      </span>
                    ))}
                  </div>
