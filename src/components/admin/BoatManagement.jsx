                ...
                {!collapsedSections['checklist'] && <div className="bg-green-50 p-5 space-y-4">
                  ...
                  <div className="border-t border-green-200 pt-4">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs font-bold text-green-900 uppercase tracking-wide">Custom Components for This Vessel</p>
                      {!locks['checklist'] && <button type="button" onClick={() => setShowCustomMaintenanceForm(p => !p)} className="flex items-center gap-1 px-2 py-1 bg-green-600 text-white rounded text-xs hover:bg-green-700"><Plus className="h-3 w-3" />Add Custom Component</button>}
                    </div>
                    {showCustomMaintenanceForm && !locks['checklist'] && (
                      <div className="bg-white border border-green-200 rounded-lg p-3 space-y-2 mb-3">
                        <p className="text-xs font-semibold text-green-800">New Custom Component</p>
                        ...
                        <div className="flex gap-2">
                          <Button type="button" size="sm" className="bg-green-600 hover:bg-green-700 text-white h-7 text-xs"
                            ...
                            onClick={() => {
                              ...
                              const prev = formData.maintenance_checklist || {};
                              const existingCustom = prev.__custom__ || [];
                              setFormData(fd => ({
                                ...fd,
                                maintenance_checklist: {
                    ...
                    {(formData.maintenance_checklist?.__custom__ || []).length > 0 && (
                      <div className="space-y-2">
                        {(formData.maintenance_checklist.__custom__ || []).map((item) => (
                          <div key={item.id} className="flex items-start gap-2 bg-white border border-green-200 rounded-lg p-3">
                            <input type="checkbox"
                              checked={!!formData.maintenance_checklist[item.id]?.checked}
                              disabled={locks['checklist']}
                              ...
                              className="mt-0.5 h-4 w-4 rounded border-green-400 accent-green-600"
                            />
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-slate-800">{item.name}</p>
                              {item.interval && <p className="text-xs text-green-700">{item.interval}</p>}
