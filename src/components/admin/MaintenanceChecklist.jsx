...
function ChecklistItem({ id, label, interval, checked, onToggle }) {
  return (
...
function ChecklistSection({ section, checklist, onToggle }) {
      ...
      {open && (
        <div className={`${c.bg} px-4 py-3`}>
          {section.items && section.items.map(item => (
            <ChecklistItem key={item.id} {...item} checked={!!checklist[item.id]} onToggle={onToggle} />
          ))}
...
export default function MaintenanceChecklist({ engineConfig, checklist = {}, onChange }) {
  ...
  const totalChecked = allItems.filter(i => checklist[i.id]).length;
  ...
  return (
    <div className="space-y-3">
      ...
      {sections.map(section => (
        <ChecklistSection key={section.id} section={section} checklist={checklist} onToggle={handleToggle} />
      ))}
    </div>
  );
}