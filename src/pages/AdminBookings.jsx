...
function AdminBookingsInner() {
  ...
  const [practiceExpeditionFilter, setPracticeExpeditionFilter] = useState('');
  ...
  React.useEffect(() => {
    if (allBoats.length > 0 && !practiceBoatFilter) {
      const isSuperAdminLocal = currentUser?.role === 'superadmin';
      const isOpAdminLocal = currentUser?.role === 'operator_admin';
      const isCharterOpLocal = currentUser?.role === 'charter_operator';
      const hasElevLocal = isSuperAdminLocal || isOpAdminLocal || isCharterOpLocal;
      const opFilterLocal = isSuperAdminLocal ? globalOperatorFilter : currentUser?.operator || 'all';
      
      const filteredOps = opFilterLocal && opFilterLocal !== 'all' ? allBoats.filter(b => (b.operator || '').toLowerCase() === opFilterLocal.toLowerCase()) : allBoats;
      
      const allowedBoats = hasElevLocal ? filteredOps : allBoats.filter(b => b.name === (currentUser?.assigned_boat || ''));
      
      if (allowedBoats.length > 0) {
        setPracticeBoatFilter(allowedBoats[0].name);
      } else {
  ...
  }, [practiceBoatFilter, allBoats, practiceExpeditionFilter]);
  ...
  const createPracticeBookingMutation = useMutation({
    mutationFn: async () => {
      ...
      let totalPrice = boat.minor_maintenance_cost || 1000;
      ...
      if (practiceExpeditionFilter) {
        ...
        expType = practiceExpeditionFilter;
      }
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #060d14 0%, #0c1f30 50%, #060d14 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 relative">
        {/* Financial Dashboard - Inline Expansion */}
                ...
                return (
                  <>
                      ...
                      <div key={s.label} className="rounded-lg px-2 py-2 flex flex-col items-center justify-center gap-1" style={{ background: s.color, border: `1px solid ${s.border}` }}>
                            ...
                            <p className={`text-[8px] text-white/60 font-medium text-center leading-tight`}>{s.label}</p>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabNavGroups isSuperAdmin={isSuperAdmin} isOperatorAdmin={isOperatorAdmin} currentUserOperator={currentUserOperator} currentUserRole={currentUser?.role} currentUserId={currentUser?.id} operatorFilter={effectiveOperatorFilter} onOperatorFilterChange={setGlobalOperatorFilter} locationFilter={globalLocationFilter} onLocationFilterChange={setGlobalLocationFilter} />
                              ...
                              <Dialog>
                                ...
                                <div className="space-y-4">
                                      <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <div><Label className="text-slate-500 text-xs">Location</Label><p className="font-medium text-sm capitalize">{selectedBooking.location?.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</p></div>
          ...
          <TabsContent value="extras">
              <div className="rounded-2xl p-6" style={{ background: 'rgba(20,184,166,0.08)', border: '1px solid rgba(20,184,166,0.2)', backdropFilter: 'blur(16px)' }}>
                <ExtrasManagementFixed allBoats={allBoats} locationFilter={globalLocationFilter} />
              </div>
            </TabsContent>
          }
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={practiceBoatFilter} onValueChange={setPracticeBoatFilter}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Select boat..." /></SelectTrigger>
                <SelectContent>
                  {(hasElevatedAccess ? filteredOperatorBoats ? allBoats.filter((b) => filteredOperatorBoats.includes(b.name)) : allBoats : allBoats.filter((b) => b.name === assignedBoat)).map((boat) =>
                  <SelectItem key={boat.id} value={boat.name}>{boat.name}</SelectItem>
                  )}
                </SelectContent>
              </Select>
